/* Найти в двумерном массиве максимальный элемент в каждой диагонали, параллельной главной. */
											...
											// Сделать встроенную функцию с передачей параметров (массивов)
...
int main()
{
	...
	do {
		...
		// Нахождение максимального элемента в каждой диагонали, параллельной главной
		for (i = 0; i < N; i++) {
			for (j = 0; j < K; j++) {
				if (i != j) {
					for (k = -K; k < N; k++) {
		...
		// Вывод максимального элемента в каждой диагонали, параллельной главной, на экран
		cout << endl;
		for (int k = 1; k < DIO; k++) {
			cout << k << ": " << max[k] << "\n";
		}
